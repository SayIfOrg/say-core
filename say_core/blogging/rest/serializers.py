...
from say_core.blogging.models import WPRegisterModel, WPSiteModel
...
class WPSiteRegisterAPIKeySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        wp_register_obj = (
            WPRegisterModel.objects.filter(code=attrs["initiation_code"]).order_by("-code_generated_at").first()
        )
        ...
        try:
            ...
            if r.status_code != status.HTTP_200_OK:
                self.fail(
                    "unsuccessful_ping", ping_url=ping_url, detail=f"status code: {r.status_code}, text: {r.text}"
                )
            if (secret_code_status := r.json().get("secret_code")) != "successful":
                self.fail(
                    "unsuccessful_ping", ping_url=ping_url, detail=f"secret code status is: {secret_code_status}"
                )
